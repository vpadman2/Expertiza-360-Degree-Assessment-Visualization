...
def assign_reviewers_staggered(num_reviews,num_review_of_reviews)
  ...
  def get_total_review_size()
      @assignment_response_size=1
    self.response_maps.each do |response_map|
      if !response_map.response.nil? then
        @assignment_response_size+=response_map.response.get_comment_size
      end

    end
      @assignment_response_size
  end

  def get_number_of_reviews_by_type()
    @number_of_reviews_by_type = Hash.new
    ...
    @REVIEW_TYPES.each do |type|
      @number_of_reviews_by_type[type] = get_total_reviews_completed_by_type(type)
    end
    @number_of_reviews_by_type
  end
  ...
  def get_total_submissions_per_assignment
    if(self.team_assignment?)
      teams = self.teams
      count = 0
